...
def custom_ODI_bats(bat, name, oname):
    # Load the dataset
    df = pd.read_csv('batting_odi.csv', encoding='latin-1')
    ...
    # Filter data for the specified teams
    filtered_df = df[(df['Opponent'] == opponent_team) & (df['Team'].str.contains('|'.join(team.split()), case=False, regex=True))]
    ...
    if len(filtered_df) < num_players:
        print(f"Caution: There are not enough batsmen against {opponent_team} for {team}.")
        return f"Caution: There are not enough batsmen against {opponent_team} for {team}."
    else:
        # Feature selection
        ...
        joblib.dump(model, 'custom_batting_odimodel.joblib')

        # Make predictions using the loaded model
        loaded_model = joblib.load('custom_batting_odimodel.joblib')
